...
export default function SignIn() {
  ...
  const { register, handleSubmit, formState: { isSubmitting, errors }, reset } = useForm<SignInFormInputs>({
    resolver: zodResolver(signInFormSchema),
  })

  async function handleLogin(data: SignInFormInputs) {
    const isLogged: boolean = await signIn({email: data.email, password: data.password})
    if (!isLogged) {
      // setNewError("Login ou senha inválidos")
  ...
  return (
    <div className='flex flex-col items-center mt-20'>
      ...
      <div className='flex flex-col flex-1 w-96 mt-60 gap-2'>
        ...
        <Link className='flex justify-center w-96 p-2 flex-1 bg-gray-400 rounded-md text-white hover:bg-gray-300' href='/users'>Criar conta</Link>
      </div>
